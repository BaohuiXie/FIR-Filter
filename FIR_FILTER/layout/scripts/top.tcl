...
# FIR_filter. The P&R flow is based on the EDI 13.1 official workshop and use the
...
set init_verilog "../syn/results/FIR_filter.mapped.v"
...
# starting from the left offset 20.0 (roughly middle of the FIR_filter module)
...
saveDesign db/FIR_filter_powerplan.enc
...
saveDesign db/FIR_filter_place.enc
...
saveDesign db/FIR_filter_cts.enc
...
saveDesign db/FIR_filter_postroute.enc
...
verifyGeometry -report reports/FIR_filter.geometry.rpt
verifyConnectivity -type all -report reports/FIR_filter.connectivity.rpt
...
report_area -out_file reports/FIR_filter.routed.area.rpt

# Power report
report_power -outfile reports/FIR_filter.routed.power.rpt

# Setup Timing Report
report_timing -machine_readable -max_paths 100 -max_slack 100.0 -min_slack -100.0 -late > FIR_filter_setup.mtarpt

# Hold Timing Report
report_timing -machine_readable -max_paths 100 -max_slack 100.0 -min_slack -100.0 -early > FIR_filter_hold.mtarpt

# ------------------------------------------------------------------------------
# Step 7b. Exports the design
# ------------------------------------------------------------------------------
# SPEF file: parastic data of wires
rcOut -spef results/FIR_filter.spef -rc_corner rc_worst

# SDF file: delay files for gates and wires
write_sdf results/FIR_filter.sdf

# *.v: verilog netlist file
saveNetlist results/FIR_filter.routed.v

# *.gds: GDS file
streamOut results/FIR_filter.gds -mapFile streamOut.map \
